 ...
 *
 * Name: Samuel Villarreal ID: 861021551
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void do_bgfg(char **argv)
{
								if(argv[1] == NULL) // err check for more than one arg
								{
                ...
                if(strcmp(argv[0], "bg")==0) // bg continue
                {
                        if(strstr(argv[1], "%") != NULL) // looks for the % of the jid
                        {
                                ...
                                if( ijid == 0 ) // err check for num
                                {
																	printf("%s argument must be a PID or %%jobid\n",argv[0]);
																	return;
																}
																if(getjobjid(jobs,ijid) ==NULL)
																{
                        ...
                        else // this block is for the pid bg
                        {
                                ...
                                if(ipid == 0)
																...
																if(getjobpid(jobs,ipid) ==NULL)
																{
																	printf("(%s): no such process\n",argv[1]);
																	return;
																}
                                getjobpid(jobs,ipid)->state = BG;
                                kill(ipid,SIGCONT); // sends signal cont
                ...
                else // fg
                {
                        ...
                        else // this block is for the pid bg
                        {
                                ...
                                kill(-ipid,SIGCONT); // sends the signal cont
                        //        printf( "[%d] (%d) %s", getjobpid(jobs,ipid)->jid ,ipid,getjobjid(jobs,ipid)->cmdline);
                        }
                        waitfg(fgpid(jobs));
...
void waitfg(pid_t pid)
{
        while( (fgpid(jobs) == pid) && getjobpid(jobs, pid)->state != ST )
        {
                sleep(1);
        }
    return;
